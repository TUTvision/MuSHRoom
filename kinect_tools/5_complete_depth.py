...
class HoleFilling_Filter(object):
	...
	"""
	def __init__(self, flag='min',radius=5,min_valid_depth=0, max_valid_depth=20000,
...
class Denoising_Filter(object):
	...
	"""
	def __init__(self, flag='modeling',theta=30, threshold=10,depth_min=0,depth_max=10000,
...
original_path = os.path.join(args.datadir, "depth")
zbuf_path = os.path.join(args.datadir, "depth_zbuf_{}".format(args.pose_scale))
complete_path = os.path.join(args.datadir, "depth_complete_{}".format(args.pose_scale))
os.makedirs(complete_path, exist_ok=True)

num = len(os.listdir(original_path))

smooth = HoleFilling_Filter(flag ='ns')
denoise = Denoising_Filter(flag = "gaussian")
for i in range(num):
    original_depth = cv2.imread(os.path.join(original_path, str(i)+".png"), -1)
    zbuf_depth = cv2.imread(os.path.join(zbuf_path, str(i)+".png"), -1)

    index = np.where(original_depth<1e-9)    
    original_depth[index[0], index[1]] = zbuf_depth[index[0], index[1]]
    

    cv2.imwrite(os.path.join(complete_path, "{}.png".format(i)), original_depth)
