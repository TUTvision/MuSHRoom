...
# load depth
depth_path = input_path / "complete_depth"
...
mean_translation = np.mean(poses, dim=0)
translation_diff = poses[:, :3, 3] - mean_translation
translation = mean_translation
up = np.mean(poses[:, :3, 1], dim=0)
...
scale = 1.0 / np.max(np.abs(poses[:, :3, 3]))
