...
@dataclass
class VRSDFStudioDataParserConfig(DataParserConfig):
    """Scene dataset parser config"""

    _target: Type = field(default_factory=lambda: VRSDFStudio)
...
@dataclass
class VRSDFStudio(DataParser):
    """SDFStudio Dataset"""

    config: VRSDFStudioDataParserConfig

    def _generate_dataparser_outputs(self, split="train"):  # pylint: disable=unused-argument,too-many-statements
        # load meta data
        meta = load_from_json(self.config.data / "meta_data.json")
        i_all = np.arange(len(meta["frames"]))

        test_data_path = Path(self.config.data).as_posix()
        test_data_name = test_data_path.split("/")[1]
        device_name = test_data_path.split("/")[2]
        test_data_path = "room_datasets/{}/{}/short_capture".format(test_data_name, device_name)
        test_data_path = Path(test_data_path)
        test_meta = load_from_json(test_data_path / "meta_data_align.json")  # dataset path
        # test_data_path = self.config.data
        # test_meta = meta

        num_images = len(meta["frames"])
        ...
        if split == "train":
            ...
            print("test")
            for i, frame in enumerate(test_meta["frames"]):
                image_filename = test_data_path / frame["rgb_path"]
                ...
                #     assert meta["has_sensor_depth"]
                #     # load sensor depth
                #     sensor_depth = np.load(test_data_path / frame["sensor_depth_path"])
                #     sensor_depth_images.append(torch.from_numpy(sensor_depth).float())

                if self.config.include_foreground_mask:
                    assert meta["has_foreground_mask"]
                    # load foreground mask
                    foreground_mask = np.array(Image.open(test_data_path / frame["foreground_mask"]), dtype="uint8")
                    foreground_mask = foreground_mask[..., :1]
                    foreground_mask_images.append(torch.from_numpy(foreground_mask).float() / 255.0)
                ...
                if self.config.include_sfm_points:
                    assert meta["has_sparse_sfm_points"]
                    # load sparse sfm points
                    sfm_points_view = np.loadtxt(test_data_path / frame["sfm_sparse_points_view"])
                    sfm_points.append(torch.from_numpy(sfm_points_view).float())
        ...
        if split == "train" and self.config.include_sensor_depth:
