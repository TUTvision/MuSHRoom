...
class VanillaPipeline(Pipeline):
    ...
    def __init__(
        ...
        # unfrozen_parameters = any(param.requires_grad for param in self.model.parameters())
        # print("Model has unfrozen parameters:", unfrozen_parameters)
    ...
    def get_eval_image_metrics_and_images(self, step: int):
        ...
        metrics_dict, images_dict = self.model.get_image_metrics_and_images(outputs, batch, 0)
    ...
    def get_average_eval_image_metrics(self, step: Optional[int] = None):
        ...
        with Progress(
            ...
            # init_R = torch.ones(3, 3).cuda()
            # init_t = torch.zeros(3).cuda()

            # rot6d = matrix_to_rotation_6d(init_R).cuda()
            # optimizer = torch.optim.SGD([rot6d, init_t], lr=1e-4, momentum=0.99)
            ...
            for camera_ray_bundle, batch in self.datamanager.fixed_indices_eval_dataloader:
                ...
                # R = rotation_6d_to_matrix(rot6d[None])[0]  # add
                ...
                # "rebuild ray bundle" add
                # camera_ray_bundle.directions = torch.einsum("ij,kqj->kqj", R, camera_ray_bundle.directions)
                # camera_ray_bundle.origins = camera_ray_bundle.origins + init_t

                #############
                outputs = self.model.get_outputs_for_camera_ray_bundle(camera_ray_bundle)

                metrics_dict, _ = self.model.get_image_metrics_and_images(outputs, batch, n)
                n += 1
