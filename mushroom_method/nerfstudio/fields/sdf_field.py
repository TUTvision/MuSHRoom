...
class SDFFieldConfig(FieldConfig):
    ...
    position_encoding_max_degree: int = 6  # 6
...
class SDFField(Field):
    ...
    def __init__(
        ...
        if self.config.encoding_type == "hash":
            ...
            self.encoding = tcnn.Encoding(
                ...
                encoding_config={
                    ...
                    "n_levels": num_levels,  # control the resolution of the encoding grid
                    "n_features_per_level": features_per_level,  # control the capacity, the number of features at each level of encoding grid.
                    "log2_hashmap_size": log2_hashmap_size,  # 2^19 hashmap size
                    "base_resolution": base_res,  # The base resolution determines the resolution of the lowest (i.e., coarsest) level of the grid. For example, if base_res is set to 16, then the lowest level of the encoding grid will have a resolution of 16x16x16 cells in 3D space. Each higher level in the grid will have twice the resolution of the previous level.
                    "per_level_scale": growth_factor,  # from coarse to finest resolution, the growth speed.
        ...
        dims = [self.config.hidden_dim for _ in range(self.config.num_layers)]
        in_dim = 3 + self.position_encoding.get_out_dim() + self.encoding.n_output_dims + self.config.geo_feat_dim + 1
        dims = [in_dim] + dims + [1 + self.config.geo_feat_dim]
        self.num_layers = len(dims)
        # TODO check how to merge skip_in to config
        self.skip_in = [4]
    ...
    def forward_geonetwork(self, inputs):
        ...
        for l in range(0, self.num_layers - 1):
            lin = getattr(self, "glin" + str(l))  # linear (71, 256), (256, 256), (256, 257)
    ...
    def forward_densitynetwork(self, inputs, sdf, geo_feature):
        """forward the geonetwork"""
        if self.use_grid_feature:
            positions = (inputs + 2.0) / 4.0  # inputs [98304, 3]
            feature = self.encoding(positions)  # [98304, 32]
        ...
        inputs = torch.cat((inputs, pe, feature, sdf, geo_feature), dim=-1)  # [98304, 71]
    ...
    def get_colors(self, points, directions, gradients, geo_features, density_feature, camera_indices):
        """compute colors"""

        # diffuse color and specular tint 镜面，色调反射
        if self.config.use_diffuse_color:
    ...
    def get_outputs(self, ray_samples: RaySamples, return_alphas=False, return_occupancy=False):
        ...
        inputs = ray_samples.frustums.get_start_positions()  # [2048, 48, 3]
        inputs = inputs.view(-1, 3)

        directions = ray_samples.frustums.directions  # [2048, 48, 3]
        directions = get_normalized_directions(directions)  # add
        directions_flat = directions.reshape(-1, 3)

        if self.spatial_distortion is not None:
            inputs = self.spatial_distortion(inputs)  # 坐标contraction, proposed in mip-nerf 360

        # compute gradient in constracted space
        inputs.requires_grad_(True)
        with torch.enable_grad():
            h = self.forward_geonetwork(inputs)
            sdf, geo_feature = torch.split(h, [1, self.config.geo_feat_dim], dim=-1)  # [n,1], [n,256]
            h_d = self.forward_densitynetwork(inputs, sdf, geo_feature)
            density, density_feature = torch.split(h_d, [1, self.config.geo_feat_dim], dim=-1)  # [n,1], [n,256]
        ...
        # density_sdf = self.laplace_density(sdf)

        rgb = self.get_colors(inputs, directions_flat, gradients, geo_feature, density_feature, camera_indices)

        #

        rgb = rgb.view(*ray_samples.frustums.directions.shape[:-1], -1)  # [2048, 48, 3]
        sdf = sdf.view(*ray_samples.frustums.directions.shape[:-1], -1)  # [2048, 48, 1]
        density = density.view(*ray_samples.frustums.directions.shape[:-1], -1)  # [2048, 48, 1]
        gradients = gradients.view(*ray_samples.frustums.directions.shape[:-1], -1)  # [2048, 48, 3]
        normals = torch.nn.functional.normalize(gradients, p=2, dim=-1)  # [2048, 48, 3]
