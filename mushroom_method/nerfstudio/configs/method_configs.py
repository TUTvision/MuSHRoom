...
method_configs["neus-facto"] = Config(
    ...
    trainer=TrainerConfig(
        # steps_per_eval_image=self.config.,
        steps_per_eval_batch=20000000,
        steps_per_eval_image=20000000,
        # steps_per_save=20000,
    ...
    pipeline=VanillaPipelineConfig(
        datamanager=VanillaDataManagerConfig(
            ...
            # ),
            camera_optimizer=CameraOptimizerConfig(
                mode="off", optimizer=AdamOptimizerConfig(lr=1e-04, eps=1e-8, weight_decay=1e-2)
            ),
        ),
    ...
    optimizers={
        "proposal_networks": {
            "optimizer": AdamOptimizerConfig(lr=1e-2, eps=1e-15),
            "scheduler": MultiStepSchedulerConfig(max_steps=80000),
        },
        "fields": {
            "optimizer": AdamOptimizerConfig(lr=5e-4, eps=1e-15),
            "scheduler": NeuSSchedulerConfig(warm_up_end=500, learning_rate_alpha=0.05, max_steps=80000),
        },
        "field_background": {
            "optimizer": AdamOptimizerConfig(lr=5e-4, eps=1e-15),
            "scheduler": NeuSSchedulerConfig(warm_up_end=500, learning_rate_alpha=0.05, max_steps=80000),
...
method_configs["nerfacto"] = Config(
    ...
    trainer=TrainerConfig(
        steps_per_eval_batch=20000000, steps_per_save=2000, max_num_iterations=40001, mixed_precision=True
    ),
    pipeline=VanillaPipelineConfig(
        datamanager=VanillaDataManagerConfig(
    ...
    optimizers={
        "proposal_networks": {
            "optimizer": AdamOptimizerConfig(lr=1e-2, eps=1e-15),
            "scheduler": MultiStepSchedulerConfig(max_steps=400000),
        },
        "fields": {
            "optimizer": AdamOptimizerConfig(lr=1e-2, eps=1e-15),
            "scheduler": MultiStepSchedulerConfig(max_steps=400000),
...
method_configs["depth-nerfacto"] = Config(
    method_name="depth-nerfacto",
    trainer=TrainerConfig(
        steps_per_eval_batch=200000000,
        steps_per_eval_image=200000000,
        steps_per_eval_all_images=2000000000,
        steps_per_save=2000,
        max_num_iterations=40001,
...
method_configs["geo-volsdf"] = Config(
    ...
    optimizers={
        ...
        "field_background": {
            "optimizer": AdamOptimizerConfig(lr=5e-4, eps=1e-15),
            "scheduler": ExponentialSchedulerConfig(decay_rate=0.1, max_steps=400000),
        },
    },
    viewer=ViewerConfig(num_rays_per_chunk=1 << 15),
    vis="viewer",
