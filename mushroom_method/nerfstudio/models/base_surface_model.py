...
class SurfaceModelConfig(ModelConfig):
    ...
    sparse_points_sdf_loss_mult: float = 0.1
...
class SurfaceModel(Model):
    ...
    def get_image_metrics_and_images(
        ...
        rgb_tmp = rgb.cpu().numpy() * 255
        rgb_tmp = cv2.cvtColor(rgb_tmp, cv2.COLOR_RGB2BGR)

        img_tmp = image.cpu().numpy() * 255
        img_tmp = cv2.cvtColor(img_tmp, cv2.COLOR_RGB2BGR)
        rgb_tmp = np.concatenate([rgb_tmp, img_tmp], axis=1)
        # cv2.imwrite(
        #     "outputs/{}/neus-facto/baseline_iphone_train_two_head_new/render/{}.png".format(self.config.file_name, n),
        #     rgb_tmp,
        # )
        ...
        # depth_tmp = combined_sensor_depth.cpu().numpy() * 255
        # cv2.imwrite(os.path.join("outputs/depth_render", str(n) + ".png"), depth_tmp)
        depth_pred = outputs["depth"][..., 0].detach().cpu().numpy()
        depth_pred = (depth_pred * 1000).astype(np.uint16)

        # zbuf_render = cv2.applyColorMap(cv2.convertScaleAbs(depth_pred / 1000, alpha=15), cv2.COLORMAP_PINK)
