...
class NerfactoModel(Model):
    ...
    def populate_modules(self):
        ...
        # self.collider = DepthCollider(near_plane=self.config.near_plane, far_plane=self.config.far_plane)
    ...
    def get_outputs(self, ray_bundle: RayBundle):
        ...
        """
        field_outputs:
        pred_normals: [n_rays, n_samples, 3] 好像是第一层的点的向量
        pred_rgb: [n_rays, n_samples, 3]
        density: [n_rays, n_samples, 1]
        normals: [n_rays, n_samples, 3] 第二次点的向量？
        """
        weights = ray_samples.get_weights(field_outputs[FieldHeadNames.DENSITY])  # [n_rays, n_samples, 1]
        weights_list.append(weights)
        ray_samples_list.append(ray_samples)  # 有3层，看起来像采样了3次，每层采样点分别是[n_rays, 256], [n_rays, 96], [n_rays, 48]

        rgb = self.renderer_rgb(rgb=field_outputs[FieldHeadNames.RGB], weights=weights)  # [4096, 3]
        depth = self.renderer_depth(weights=weights, ray_samples=ray_samples)  # [4096, 1]
        accumulation = self.renderer_accumulation(weights=weights)  # [4096, 1]
    ...
    def get_image_metrics_and_images(
        self, outputs: Dict[str, torch.Tensor], batch: Dict[str, torch.Tensor], n
        ...
        rgb_tmp = (rgb[0, :, :, :]).permute(1, 2, 0).cpu().numpy() * 255
        ...
        # metrics_dict = {"mse": mse, "psnr": float(psnr.item()), "ssim": float(ssim)}  # type: ignore
