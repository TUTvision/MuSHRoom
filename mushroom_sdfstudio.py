"""
mushroom dataset converts to sdfstudio format
...
def main(args):
    ...
    long_cam_params = json.load(open(input_dir / "long_capture" / "transformations_colmap.json"))
    short_cam_params = json.load(open(input_dir / "short_capture" / "transformations_colmap.json"))
    ...
    for frame in short_frames:
        ...
        if "depth_file_path" in frame:
            ...
            depth_paths.append(depth_path)
        if "mono_depth_file_path" in frame:
            mono_depth_path = input_dir / "short_capture" / frame["mono_depth_file_path"]
            assert mono_depth_path.exists()
            mono_depth_paths.append(mono_depth_paths)
    ...
    for idx, (valid, pose, image_path) in enumerate(tqdm(zip(valid_poses, poses, image_paths))):
        ...
        # save rgb image
        out_img_path = output_dir / Path(image_path.parents[1].name + "_" + image_path.stem + "_rgb.png")
        if args.save_imgs:
        ...
        frame = {
            ...
            "intrinsics": cam_intrinsics[0].tolist() if "fl_x" in long_cam_params else cam_intrinsics[idx].tolist(),
        }

        if sensor_depth:
            depth_path = depth_paths[idx]
            out_depth_path = output_dir / Path(image_path.parents[1].name + "_" + image_path.stem + "_sensor_depth.png")
            # load depth
            if args.save_imgs:
        ...
        # need to first prepare the normals
...
if __name__ == "__main__":
    ...
    parser.add_argument("--output_dir", required=True, help="path to output data directory")
    parser.add_argument(
        "--scene-type",
        dest="scene_type",
        required=True,
    ...
    parser.add_argument("--save-imgs", action="store_true", help="Use normal from mono depth")
